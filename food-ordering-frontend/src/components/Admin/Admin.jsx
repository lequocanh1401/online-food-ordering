...
export const Admin = () => {
    ...
    const isSuperAdmin = auth.user && auth.user.role === "ROLE_ADMIN";
    ...
    useEffect(() => {
        if (jwt && auth.user && auth.user.role === "ROLE_RESTAURANT_OWNER") {
    ...
    if (!isSuperAdmin && (!restaurant?.usersRestaurant || !restaurant?.usersRestaurant?.id)) {
        return (
