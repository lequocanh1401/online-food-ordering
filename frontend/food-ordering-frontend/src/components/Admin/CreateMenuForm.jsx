...
export const CreateMenuForm = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            // Đóng gói dữ liệu chuẩn 100% theo DTO của Spring Boot
            const data = {
                name: values.name,
                description: values.description,
                price: Number(values.price), // Ép kiểu giá tiền thành Số
                images: [values.images],
                restaurantId: restaurant.usersRestaurant?.id,
                category: { id: values.category }, // Đổi tên biến chuẩn
            ...
            <Box className='max-w-3xl w-full bg-gray-800 p-8 rounded-lg shadow-xl'>
                ...
                <form onSubmit={formik.handleSubmit} className='space-y-4'>
                    ...
                    <div className='flex gap-4'>
                        <TextField fullWidth name="price" label="Giá (đ)" value={formik.values.price} onChange={formik.handleChange} sx={{ input: { color: 'white' }, label: { color: 'gray' } }} />
                        <FormControl fullWidth>
                            ...
                            <Select name="category" value={formik.values.category} onChange={formik.handleChange} sx={{ color: 'white' }}>
                                {restaurant.categories.map((item) => <MenuItem key={item.id} value={item.id}>{item.name}</MenuItem>)}
                            </Select>
                        </FormControl>
                    </div>
                    ...
                    <FormControl fullWidth>
                        ...
                        <Select multiple name="ingredients" value={formik.values.ingredients} onChange={formik.handleChange} input={<OutlinedInput label="Nguyên liệu" />} renderValue={(selected) => (
                            ...
                            {ingredients.ingredients.map((item) => <MenuItem key={item.id} value={item}>{item.name}</MenuItem>)}
                        </Select>
                    </FormControl>
